...
class ControllerMahardhiBlogArticle extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_mahardhi_blog_article->addArticle($this->request->post);
			...
			$this->response->redirect($this->url->link('mahardhi_blog/article', 'user_token=' . $this->session->data['user_token'] . $url, true));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_mahardhi_blog_article->editArticle($this->request->get['article_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $article_id) {
				$this->model_mahardhi_blog_article->deletearticle($article_id);
			}
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('mahardhi_blog/article/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('mahardhi_blog/article/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			$data['articles'][] = array(
				...
				'edit'       		=> $this->url->link('mahardhi_blog/article/edit', 'user_token=' . $this->session->data['user_token'] . '&article_id=' . $result['article_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('mahardhi_blog/article', 'user_token=' . $this->session->data['user_token'] . '&sort=pd.name' . $url, true);
		$data['sort_status'] = $this->url->link('mahardhi_blog/article', 'user_token=' . $this->session->data['user_token'] . '&sort=p.status' . $url, true);
		$data['sort_date_available'] = $this->url->link('mahardhi_blog/article', 'user_token=' . $this->session->data['user_token'] . '&sort=p.date_available' . $url, true);
		$data['sort_date_modified'] = $this->url->link('mahardhi_blog/article', 'user_token=' . $this->session->data['user_token'] . '&sort=p.date_modified' . $url, true);
		$data['sort_order'] = $this->url->link('mahardhi_blog/article', 'user_token=' . $this->session->data['user_token'] . '&sort=p.sort_order' . $url, true);
		...
		$this->response->setOutput($this->load->view('mahardhi_blog/article_list', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['article_id'])) {
			$data['action'] = $this->url->link('mahardhi_blog/article/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('mahardhi_blog/article/edit', 'user_token=' . $this->session->data['user_token'] . '&article_id=' . $this->request->get['article_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('mahardhi_blog/article', 'user_token=' . $this->session->data['user_token'] . $url, true);

		if (isset($this->request->get['article_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$article_info = $this->model_mahardhi_blog_article->getArticle($this->request->get['article_id']);
		}
		...
		if (isset($this->request->post['article_description'])) {
			...
			$data['article_description'] = $this->model_mahardhi_blog_article->getArticleDescriptions($this->request->get['article_id']);
		} else {
		...
		if (isset($this->request->post['article_store'])) {
			...
			$data['article_store'] = $this->model_mahardhi_blog_article->getArticleStores($this->request->get['article_id']);
		} else {
		...
		$this->response->setOutput($this->load->view('mahardhi_blog/article_form', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'mahardhi_blog/article')) {
		...
		if (utf8_strlen($this->request->post['keyword']) == 0) {
			$this->language->load('mahardhi_blog/seotranslit');
		...
		if (utf8_strlen($this->request->post['keyword']) > 0) {
			$this->load->model('mahardhi_blog/url_alias');

			$url_alias_info = $this->model_mahardhi_blog_url_alias->getUrlAlias($this->request->post['keyword']);
